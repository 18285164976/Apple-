...
/*
 ...
 作者：清蘂翅膀的技术
 链接：https://www.jianshu.com/p/bdc5119a7312
 來源：简书
...
@property(nonatomic,strong) UIImageView *imageViewNormal;
...
- (void)viewDidLoad {
    ...
    self.imageNormal = [UIImage imageNamed:@"IMG_0220.JPG"];
    
    self.imageViewNormal = [[UIImageView alloc] init];
    [self.view addSubview:self.imageViewNormal];
    [self.imageViewNormal mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.view).offset(64);
        make.left.right.equalTo(self.view);
        make.height.equalTo(@200);
    }];
    self.imageViewNormal.image = self.imageNormal;
    
    GPUImageFalseColorFilter *filter =  [[GPUImageFalseColorFilter alloc] init];
    self.NewImage = [self filterImage:filter image:self.imageNormal];
    
    
    //加载出来
    self.imageViewNew = [[UIImageView alloc] initWithImage:self.NewImage];
    [self.view addSubview:self.imageViewNew];
    [self.imageViewNew mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self.view);
        make.top.equalTo(self.imageViewNormal.mas_bottom).offset(10);
        make.height.equalTo(@200);
    }];
...
-(UIImage *)filterImage:(GPUImageFilter *)imageFilter image:(UIImage *)image
{
    //设置要渲染的区域
